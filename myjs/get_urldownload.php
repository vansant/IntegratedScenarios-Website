	...
	if($productt=="macav1metdata"){
 	...
 	if($productt=="vic"){
                if($Files=='THREDDS'){
                        ...
                        $mymonthlyPath = "{$NKNTHREDDS}/fileServer/NWCSC_INTEGRATED_SCENARIOS_ALL_HYDROLOGY_VIC";
                }
                $myAggPath="{$NKNTHREDDS}/ncss/grid";
                $regionn="WUSA";
       ...
       if($productt=="ulm"){
                if($Files=='THREDDS'){
                        ...
                        $mymonthlyPath = "{$NKNTHREDDS}/fileServer/NWCSC_INTEGRATED_SCENARIOS_ALL_HYDROLOGY_ULM";
                }
                $myAggPath="{$NKNTHREDDS}/ncss/grid";
                $regionn="WUSA";
	...
	foreach($_POST["model"] as $model){
		...
		foreach($_POST["varr"] as $varr){
			if($varr=="tasmax" ||$varr=="tasmin" ){
				...
				if($productt=='vic'){
					$temparray=explode('-',$varr);
				 	$varr_longname=$temparray[0];
				}else{
				 	$varr_longname=$varr;
				}
			}	
			...
			foreach($_POST["timee"] as $timee){
			   ...
			   foreach($_POST["freqq"] as $freqq){
				...
				if($varr=="" || $productt=="mc2_WithoutFireSuppression"|| $productt=="mc2_WithFireSuppression"){ //true for mc2
					$timearray=explode('_',$timee);
					$expp=$timearray[0];
					if($expp=="historical"){
						$fileName="{$productt}_PRISM_{$timee}_{$regionn}{$freqq}.nc";
						$urlPath="{$dailyPath}/PRISM"; 
					}else{
						$fileName="{$productt}_{$model}_{$timee}_{$regionn}{$freqq}.nc";
					}
				}else{
				...
				if($domainn =="rectangle"){
	                       ...
	                       if($domainn=="rectangle" || $domainn=="point"){
                                        ...
                                        //$yrstart=$timearray[2];
                                        //$yrend=$timearray[3];
                                        //$datequerystring="&temporal=range&time_start={$yrstart}-01-01T00%3A00%3A00Z";
                                        //$datequerystring= "{$datequerystring}&time_end={$yrend}-12-31T00%3A00%3A00Z";
                                        $datequerystring="&temporal=all";
                                }
                                if($domainn=="rectangle"){
                                        $querystring = "{$querystring}{$datequerystring}&accept={$extensionn}&point=false";
                                }
                                else if($domainn=="point") {
                                        $querystring = "{$querystring}{$datequerystring}&accept={$extensionn}&point=true";
                                }else{
                                        $querystring="";
                                }
